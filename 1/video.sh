...
# To find the number related to your prefered device please type "arecord -l" in another terminal prompt.
	...
	echo "Enter you screen device number (e.g. 1 or 0 or other number) and press ENTER. You can find this number by writing
[arecord -l] in your shell :"

	read linscreen

# Look for your audio device number (e.g. 1 or 0 or other number) and enter it when asked for. The script will adapt the command accorded to your system.
# To find the number related to your prefered device please type "arecord -l" in another terminal prompt.
...
# This part will adapt the video record command accorded to the informations you provided.	
	...
	ffmpeg  -video_size "$reslinux" -framerate 30 -f x11grab -i "$disp"."$linscreen" -f alsa -ac 2 -i hw:"$lindevice" -c:v libx264 -c:a aac -qp 0 -preset ultrafast $path$name.mp4
	echo "video successfully recorded"
	
	elif [ "$os" == "Darwin" ] && [ -n $path ] && [ -n $name ];
	then

	ffmpeg -f avfoundation -capture_cursor 1 -capture_mouse_clicks 1 -i "$resosx":"$osxdevice" -c:v libx264 -qp 0 -preset ultrafast $path$name.mp4
